// src/broadcast.js —— 播报文案引擎（移植自小程序 utils/broadcast.js，纯函数）
...
// 每日开场：P1 逾期 → P2 坚持 → P3 昨日 → P4 兜底（只挑最高一档）
export function dailyOpen({ overdueList = [], streak = 0, yDone = [] }, petName = '橘小满') {
...
export function completeReact({ overdue = false, openedP2P3 = false }, petName = '橘小满') {
  if (overdue) {
...
export function milestone(streak, petName = '橘小满') {
